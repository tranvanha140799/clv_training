...
import { Permission, Role, User } from '@/redux/common/types';
...
const roleColumns = (
  ...
  },
  {
    ...
    render: (values: Permission[], record: Role) =>
      currentRoleId && currentRoleId === record.id ? (
        <Select
          mode="multiple"
          style={{ width: '100%' }}
  ...
  },
  {
    ...
    render: (value: any, record: User) => (
      <Button
        ...
        type="default"
      >
        {currentRoleId && currentRoleId === record.id ? 'Cancel' : 'Edit Role'}
      </Button>
    ),
  },
];

const RoleManagement: NextPage<ProfileProps> = ({}) => {
  const {
    data: roles,
